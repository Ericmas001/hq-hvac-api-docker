...
import traceback
import os
from const import PIN

path = "/config/last_config.json"
data = None

def get_index(app):
    return "HVAC CONTROLLER is HERE"
...
def power_off(app):
    hvac = Mitsubishi(PIN, LogLevel.ErrorsOnly)
    hvac.power_off()
...
def __send_command(app, command):
    try:
        hvac = Mitsubishi(PIN, LogLevel.Minimal)
        args = dict(
